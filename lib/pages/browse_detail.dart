...
class _TestDBrowselPageState extends State<BrowseDetailPage> {
  ...
  @override
  void initState() {
    fbAnalytics.logScreenView('${widget.novel} Detail Page');
    super.initState();
    auth = AuthFirebase();
    getNovel();
    isNovelDownloaded();
  ...
  Future<void> requestNotificationPermission() async {
    ...
    if (await Permission.notification.status.isGranted) {
      noti.showLocalNoti('download'.i18n(), 'download_success'.i18n());
      downloadNovel();
      downloadAndSaveImage(novel["img"], novel["name"]);
      setState(() {
        isDownloaded = true;
      });
      fbAnalytics.logDownloadEvent(
          'Downloaded_${widget.novel.toString().split(' ').join('_')}',
          FirebaseAuth.instance.currentUser!.email?.split('@').toString(),
          widget.novel);
    } else {
                                                                                ...
                                                                                onPressed: () async {
                                                                                  ...
                                                                                  for (var map in ListComm) {
                                                                                    if (map.keys.length == currentDoc.keys.length && map.keys.every((key) => map[key] == currentDoc[key])) {
                                                                                      ...
                                                                                      fbAnalytics.logReviewEvent('Delete_Review_${novel['name'].toString().split(' ').join('_')}', FirebaseAuth.instance.currentUser!.email?.split('@').toString(), currentRate, reviewController.text);
                                                                                      final snackBar = SnackBar(
                                                                                        ...
                                                                                        action: SnackBarAction(
                                                                                          label: 'dismiss',
                                                                                          onPressed: () {
                                                                                            ScaffoldMessenger.of(widgetContext).hideCurrentSnackBar();
                                                                                          },
                                                                                        ),
                                                                                      );
